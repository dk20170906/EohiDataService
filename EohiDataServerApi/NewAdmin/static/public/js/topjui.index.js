...
function generateMenu(menuId, systemName) {
    if (menuId != undefined) {
        ...
        var url = _ctx + "../Data/Menu/MenuJsonDataByUserGuid";
        $.get(
            url, {"pguid": ""}, // 获取第一层目录
            function (data) {
                if (data == "") {
                ...
                $.each(data, function (i, e) {// 循环创建手风琴的项
                    ...
                    $('#RightAccordion').iAccordion('add', {
                        fit: false,
                        title: e.text,
                        content: "<ul id='tree" + e.guid + "' ></ul>",
                        border: false,
                        selected: isSelected,
                        iconCls: e.iconCls
                    });
                    //$.parser.parse();
                    $.get(url, { "pguid": e.guid },                   //"./json/menu/menu_" + e.id + ".json", function (data) {// 循环创建树的项
                        function (data) {
                            $("#tree" + e.guid).tree({
                            data: data,
                            lines: false,
                            ...
                            onBeforeExpand: function (node, param) {
								//alert("./json/menu/menu_" + node.id + ".json");
                                //debugger;
                                $("#tree" + e.guid).tree('options').method = "get";
                                $("#tree" + e.guid).tree('options').url = url + "?pguid=" + node.guid;
                            },
                            onClick: function (node) {
                                if (node.url) {
                                    ...
                                    if (node.state == "closed") {
                                        $("#tree" + e.guid).tree('expand', node.target);
                                    } else if (node.state == 'open') {
                                        $("#tree" + e.guid).tree('collapse', node.target);
                                    }
                                }
                            }
                        });
...
function modifyPwd() {
    var opts = {
        ...
        height: 300,
        iconCls: 'fa fa-key',
        href: '/NewAdmin/html/user/modifyPassword.html',
        buttons: [{
            text: '确定',
            iconCls: 'fa fa-save',
    ...
    　　　　},
        complete: function (XMLHttpRequest, textStatus, data) {
            ...
            if (result != undefined && result != "") {
                ...
                if (result.sessionstatus != undefined) {

                    if (result.sessionstatus == "timeout") {

                        // +
                        var top = getTopWinow(); //获取当前页面的顶层窗口对象
                        $.messager.alert('我的消息', '登录超时-请重新登录！', 'info');
                        top.location.href = "http://" + window.location.host + "/NewAdmin/login.html"; //跳转到登陆页面 对多服务器同样适用
                    }
                }
            }
